...
heap_t* create_heap(uint32_t start_addr, uint32_t end_addr, uint32_t max,
  ...
  heap->index = place_ordered_array((void*)start_addr, HEAP_INDEX_SIZE,
                                    &header_t_less_than);
...
void* alloc(uint32_t size, uint8_t page_align, heap_t* heap) {
  ...
  header_t* orig_hole_header =
      (header_t*)lookup_ordered_array(iter, &heap->index);
  ...
  // write a new hole after the allocated block if the new whole has a positive
  // size
  if (orig_hole_size - new_size > 0) {
    ...
    footer_t* hole_footer = (footer_t*)((uint32_t)hole_header + orig_hole_size -
